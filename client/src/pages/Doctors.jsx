      ...
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
        ...
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex' :'hidden sm:flex'}`}>
          <p onClick={()=>speciality === 'General physician' ? navigate('/doctors') : navigate("/doctors/General physician")} 
             className={`w-[94vw] sm:w-auto pl-3 pr-16 border border-gray-500 py-1.5 rounded transition-all cursor-pointer 
        ...
        <div className="w-full grid grid-cols-auto gap-4">
          {filterDoc.map((item, idx) => (
            <div
              ...
              key={idx}
              className="border border-blue-200 cursor-pointer rounded-xl overflow-hidden
